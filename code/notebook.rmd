...
# CAUTION: We are merging a weekly timeseries togehter with a monthly timeseries. When creating distributions/doing t-tests keep that in mind and maybe use the "smaller" dataframes.
# Because of the problem above, we create another dataframe with all employee related dataframe with only the wages on the monthly timeseries
volunteer_endperiod_wage <- merge(volunteer_endperiod, wage, by="personid", all=TRUE)

volunteer_endperiod_wage<-volunteer_endperiod_wage %>% filter(is.finite(grosswage), is.finite(bonustotal), is.finite(basewage))
...
# when using this dataframe we have to be cautious as attitude has way less columns than attitude n(attitude_performance) > n(performance), as a result of the outer-join, both table have combinations of personid, year_weak the otherone does not
...
radar_by_group(df, group_col="quitjob", include_group_indices=c(TRUE, FALSE), title="Quitter vs non-quitters", show_full_attitude = FALSE)
...
df <- volunteer_endperiod_attitude_performance_wage
radar_by_group(df, group_col="group_index", include_group_indices=c(7,16,40,48,60), title="Quitter group characteristics", save=TRUE, show_full_attitude = FALSE)
```
...
ggcorrplot(cm, lab = FALSE)
# radar plot
radar_by_group(df, group_col="group_index", include_group_indices=c(7), title="Group 7 characteristics", show_full_attitude = FALSE)
```
...
ggcorrplot(cm, lab = FALSE)
# radar plot
radar_by_group(df, group_col="group_index", include_group_indices=c(16), title="Group 16 characteristics", show_full_attitude = FALSE)
```
...
ggcorrplot(cm, lab = FALSE)
# radar plot
radar_by_group(df, group_col="group_index", include_group_indices=c(40), title="Group 40 characteristics", show_full_attitude = FALSE)
```
...
ggcorrplot(cm, lab = FALSE)
# radar plot
radar_by_group(df, group_col="group_index", include_group_indices=c(48), title="Group 48 characteristics", show_full_attitude = FALSE)
```
...
ggcorrplot(cm, lab = FALSE)
# radar plot
radar_by_group(df, group_col="group_index", include_group_indices=c(60), title="Group 60 characteristics", show_full_attitude = FALSE)
```
...
radar_by_group(df, group_col="quitjob", include_group_indices=c(TRUE), title="General quitter profile", show_full_attitude = FALSE)
...
radar_by_group(df, group_col="quitjob", include_group_indices=c(FALSE), title="General non-quitter profile", show_full_attitude = FALSE)
...
#H0: The mean(positivity) of top performes is equal to the mean(positivity) of the rest
#H1: The mean(positivity) of top performes greater than the mean(positivity) of the rest
